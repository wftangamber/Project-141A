...
d_rate_table=d_rate_table %>%
  transmute(Cumulative_cases=Cumulative_cases,WHO_region=WHO_region,Population2020=`Population..2020.`,Pop_yearly_change=as.numeric(`Yearly.Change`),Net.Change=Net.Change,population_density=`Density..P.KmÂ².`,land_area=`Land.Area..KmÂ².`,Migrant_net_change=`Migrants..net.`,Fert_rate=as.numeric(`Fert..Rate`), median_age=as.numeric(`Med..Age`),urban_population_percent=as.numeric(`Urban.Pop..`),World_Share=as.numeric(`World.Share`),GDP_per_capita=as.numeric(`GDP per Capita $PPP 2019`),health_expenditure_of_GDP=as.numeric(`health expenditure in percent of GDP`))
...
index_50percent= createDataPartition(d_rate_table$Cumulative_cases,p=0.5,list=FALSE,times=1)

#test and training.
train=d_rate_table[index_50percent,]
test=d_rate_table[-index_50percent,]


#train=d_rate_table %>% sample_frac(0.8)
#test=d_rate_table %>% setdiff(train)
x_train=model.matrix(Cumulative_cases~.,train)[,-1]
x_test=model.matrix(Cumulative_cases~.,test)[,-1]
y_train=train %>% select(Cumulative_cases) %>% unlist() %>% as.numeric()
y_test=test %>% select(Cumulative_cases) %>% unlist() %>% as.numeric()


lasso_mod=glmnet(x_train,y_train,alpha=1)
plot(lasso_mod)
cv.train=cv.glmnet(x_train,y_train,alpha=1,type.measure = "mse")
plot(cv.train)
